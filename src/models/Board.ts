...
export class Board {
    cells: Cell[][] = [];
    lossBlackFigures: Figure[] = [];
    lossWhiteFigures: Figure[] = [];
    ...
    public highlightCells(selectedCell: Cell | null) {
        for (let i = 0; i < this.cells.length; i++) {
            ...
            for (let j = 0; j < row.length; j++) {
                const target = row[j]
                const isCanMove = selectedCell?.figure?.canMove(target);
                target.available = !!isCanMove;
            }
        }
    }
    ...
    public getCopyBoard(): Board {
        ...
        newBoard.lossBlackFigures = this.lossBlackFigures;
        newBoard.lossWhiteFigures = this.lossWhiteFigures;
        return newBoard;
    }
    ...
    // public addFisherFigures() {} // если хотим играть в шахматы фишера
